...
class ResponseAttention(nn.Module):
    def __init__(self, num_feature_maps=4):
        ...
        self.attention_weights = nn.Linear(num_feature_maps, num_feature_maps)

        self.conv_layer = nn.Conv2d(num_feature_maps, num_feature_maps, kernel_size=3, padding=1)
        self.avgpool = nn.AvgPool2d(3)
        
    def forward(self, feature_maps):
        batch_size = feature_maps.shape[0]
        og_feature_maps = feature_maps
        # print(f"Shape: {feature_maps.shape}")
        feature_maps = feature_maps.squeeze(2)
        feature_maps = F.relu(self.conv_layer(feature_maps))
        feature_maps = self.avgpool(feature_maps)

        # Flatten the spatial dimensions for computing attention
        flattened_maps = feature_maps.view(batch_size, self.num_feature_maps, -1)  # shape: (batch_size, num_feature_maps, channels * height * width)
        
        # Compute attention scores
        attention_scores = self.attention_weights(flattened_maps.mean(dim=-1))  # shape: (batch_size, num_feature_maps)
        # print(f"before softmax: {attention_scores.shape}")
        # Apply softmax to get attention weights
        attention_weights = F.softmax(attention_scores, dim=1)  # shape: (batch_size, num_feature_maps)
        # print(f"after softmax: {attention_weights.shape}")
        # Expand the weights to match the feature maps dimensions
        attention_weights = attention_weights.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)  # shape: (batch_size, num_feature_maps, 1, 1, 1)
        # print(f"after unsqueeze: {attention_weights.shape}")
        ...
        weighted_feature_maps = og_feature_maps * attention_weights  # element-wise multiplication
        
        # Combine the weighted feature maps
        combined_feature_map = weighted_feature_maps.sum(dim=1)  # shape: (batch_size, channels, height, width)
...
class MIPCLIP(nn.Module):
    ...
    def __init__(self, bert_tokenizer='clip', backbone='clip-RN50', max_query_len=20):
        ...
        self.response_attention = ResponseAttention()   
    ...
    def forward(self, x, word_id):
        ...
        seg_final_out1 = torch.stack([seg_final_out1, seg_final_out2, seg_final_out3, seg_final_out4], dim=1)

        seg_final_out1 = self.response_attention(seg_final_out1)
