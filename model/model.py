...
class TRIS(nn.Module):
    ...
    def forward(self, x, word_id):
        ...
        pooled_output1 = self.global_avg_pool(seg_out1)
        pooled_output2 = self.global_avg_pool(seg_out2)
        pooled_output3 = self.global_avg_pool(seg_out3)
        pooled_output4 = self.global_avg_pool(seg_out4)
        
        pooled_output1 = pooled_output1.view(pooled_output1.size(0), -1)
        pooled_output2 = pooled_output2.view(pooled_output2.size(0), -1)
        pooled_output3 = pooled_output3.view(pooled_output3.size(0), -1)
        pooled_output4 = pooled_output4.view(pooled_output4.size(0), -1)

        merged = torch.cat([pooled_output1, pooled_output2, pooled_output3, pooled_output4], dim=1)
...
def criterion(bbox_out, target):
    ...
    return nn.L1Loss()(bbox_out, target)
